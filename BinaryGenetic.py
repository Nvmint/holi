        ...
        while generacion <= self._generaciones:
            ...
            while len(hijos) < len(self._individuos):
                ...
                while padre1 == padre2:
                    padre2 = self.ruleta()
                h1, h2 = self.cruza(
                    self._individuos[padre1], self._individuos[padre2])
                hijos = np.append(hijos, [h1])
                hijos = np.append(hijos, [h2])
            self.mutacion(hijos)
            self._individuos = np.copy(hijos)
            self._individuos[np.random.randint(len(self._individuos))] = copy.deepcopy(
                self._mejor_historico)
            if generacion % 100 == 0:
...
def main():
    ...
    valores = [55, 34, 28, 30, 80, 3, 28, 24,
               21, 43, 54, 12, 21, 11, 6, 21, 28]
    capacidad = 80
    ...
    ag = AG(individuos, alelos, tamano_gen,
            generaciones, factor_mutacion, mochila)
    ag.run()
